...
### やること
  ...
  * 全員から感想を聞く
    * いきなりディスカッションをはじめても萎縮してしまう
  * 事前質問に回答する
    * 複数プラットフォームで動作する実装で、OSごとに処理を変える必要がある場合の読みやすい書き方について
    * コードフォーマッタ・コーディングルールとの折り合い付けについて
    * コードにおける日本語について（エラーメッセージ、テストの関数名）
    * C++のVerupで書けるようになった新しい書き方や、ラムダなどを使用した書き方との折り合い付けについて
    * 年代物（10年以上前）のコードを改修する際の指針について 
  * ディスカッション
    * このように書くとリーダブルと言える、これくらいなら個々人でも無理なく実践できる、という基準を見出す。
    * 先の基準のリーダブルなコードを「チームで」無理なく実践する方法を考える。
  * ディスカッションを実践するために各人が取り組むアクションを設定する
    * 講座終了後からすぐに取り組める具体的な行動を設定する。
    * この場で得た知見をその場限りの物に終わらせずに、社内に根付かせていくきっかけとする。
